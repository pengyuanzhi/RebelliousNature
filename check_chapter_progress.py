...
# 读取正文
with open('D:/AI/homework/ClaudeCode/RebelliousNature/逆命问道_正文.md', 'r', encoding='utf-8') as f:
    content = f.read()

# 提取第一卷所有章节
pattern = r'## (第[五三四六七八九十百零壹壹贰贰叁叁肆肆伍伍陆陆柒柒捌捌玖玖拾拾一二三四五六七八九十百]+章|第\d+章) ([^\n]+)\n(.*?)(?=## 第|$)'
...
for match in matches[:74]:
    ...
    chinese_count = len([c for c in body if '\u4e00' <= c <= '\u9fff'])
    chapters.append({
        'chapter': chapter_num,
        'title': title.strip(),
